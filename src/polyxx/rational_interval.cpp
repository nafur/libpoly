...
RationalInterval::RationalInterval(const Rational& a,
                                   ...
                                   bool b_open)
{
  lp_rational_interval_construct(get_internal(),
                                 a.get_internal(),
                                 a_open ? 1 : 0,
                                 b.get_internal(),
                                 b_open ? 1 : 0);
...
RationalInterval::RationalInterval(const DyadicRational& a,
                                   ...
                                   bool b_open)
{
  lp_rational_interval_construct_from_dyadic(get_internal(),
                                             a.get_internal(),
                                             a_open ? 1 : 0,
                                             b.get_internal(),
                                             b_open ? 1 : 0);
...
lp_rational_interval_t* RationalInterval::get_internal() { return &mInterval; }
...
bool contains(const RationalInterval& ri, const AlgebraicNumber& an) {
  return lp_rational_interval_contains_algebraic_number(ri.get_internal(), an.get_internal());
}
bool contains(const RationalInterval& ri, const DyadicRational& dr) {
  return lp_rational_interval_contains_dyadic_rational(ri.get_internal(), dr.get_internal());
}
bool contains(const RationalInterval& ri, const Integer& i) {
  return lp_rational_interval_contains_integer(ri.get_internal(), i.get_internal());
}
bool contains(const RationalInterval& ri, const Rational& r) {
  return lp_rational_interval_contains_rational(ri.get_internal(), r.get_internal());
}
bool contains(const RationalInterval& ri, const Value& v) {
  return lp_rational_interval_contains_value(ri.get_internal(), v.get_internal());
}
