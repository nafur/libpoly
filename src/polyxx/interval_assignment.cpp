...
namespace poly
{
  ...
  lp_interval_assignment_t *IntervalAssignment::get_internal() { return &mAssignment; }
  const lp_interval_assignment_t *IntervalAssignment::get_internal() const { return &mAssignment; }

  void IntervalAssignment::set(const Variable &var, const Interval &interval)
  {
    lp_interval_assignment_set_interval(get_internal(), var.get_internal(), interval.get_internal());
  }
  void IntervalAssignment::unset(const Variable &var)
  {
    lp_interval_assignment_set_interval(get_internal(), var.get_internal(), nullptr);
  }
  Interval IntervalAssignment::get(const Variable &var) const
  {
    return Interval(lp_interval_assignment_get_interval(get_internal(), var.get_internal()));
  }
