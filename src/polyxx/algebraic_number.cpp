...
AlgebraicNumber::AlgebraicNumber(UPolynomial&& poly, const DyadicInterval& di)
{
  lp_algebraic_number_construct(
      get_internal(), poly.release(), di.get_internal());
}
AlgebraicNumber::AlgebraicNumber(const UPolynomial& poly,
                                 const DyadicInterval& di)
{
  lp_algebraic_number_construct(
      get_internal(), UPolynomial(poly).release(), di.get_internal());
}
AlgebraicNumber::~AlgebraicNumber()
{
...
{
  return lp_algebraic_number_cmp_integer(lhs.get_internal(), rhs.get_internal())
         == 0;
}
bool operator!=(const AlgebraicNumber& lhs, const Integer& rhs)
{
